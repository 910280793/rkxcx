...
Page({
            ...
            success: function (res) {
              ...
              wx.login({
                success: res => {
                  ...
                  var APPID = 'wxfafafd22f1f42769'
                  var SECRET = 'adb4f3c51a6583a70856563c33911985'
                  wx.request({
                    url: 'https://api.weixin.qq.com/sns/jscode2session?appid=' + APPID + '&secret=' + SECRET + '&js_code=' + res.code + '&grant_type=authorization_code',
                    success: res => {
                      // 获取到用户的 openid
                      app.GLOBAL_DATA.OPENID = res.data.openid
                      var u=app.GLOBAL_DATA.USER_INFO
                  
                      wx.request({
                        url: app.GLOBAL_DATA.BASE_URL+'user/addUser',//注意改成自己的服务器请求地址哦！
                        data: {
                          openid:res.data.openid,
                          avaterurl: u.avatarUrl,
                          city: u.city,
                          country: u.country,
                          gender: u.gender,
                          language: u.language,
                        ...
                        success: function (res) {
                          ...
                          console.log(app.GLOBAL_DATA.USER_INFO2)
                        },
                        fail: function (error) {
                          console.log(error);
                        }
  ...
  bindGetUserInfo: function (e) {
    ...
    if (e.detail.userInfo) {
      ...
      const app=getApp();
      this.login();
      app.GLOBAL_DATA.USER_INFO = e.detail.userInfo
      console.log("app+" + app.GLOBAL_DATA.USER_INFO.nickName)
  ...
  bindtest: function () {
    wx.request({
      url: app.GLOBAL_DATA.BASE_URL+'test/list',  //本地服务器地址
      data: {
  ...
  bannerChange: function (e, delay) {
    let b = this.data.banners[e.detail.current]
    if (!b) return
    setTimeout(() => {
